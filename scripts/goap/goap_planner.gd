...
func get_plan(goal: GoapGoal, blackboard = {}) -> Array:
	print("Blackboard %s: %s" % [_agent.name, blackboard])
	print("Goal %s: %s" % [_agent.name, goal.get_id()])
	WorldState.console_message("Goal: %s" % goal.get_id())
...
func _find_best_plan(goal, desired_state, blackboard):
	...
	var root = {"action": goal, "state": desired_state, "children": []}
...
func _build_plans(step, blackboard):
	...
	for action in _actions:
		...
		if should_use_action:
			...
			var s = {"action": action, "state": desired_state, "children": []}
